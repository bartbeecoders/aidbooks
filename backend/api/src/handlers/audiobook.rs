...
pub struct CreateAudiobookRequest {
    ...
    /// When `true`, run the songbook outline pipeline: the topic is
    ...
    pub is_songbook: Option<bool>,
    /// Number of song snippets to splice into the published audio.
    ...
    /// Adopt snippets the user already heard via
...
pub struct AutoPublishRequest {
    ...
    #[serde(default)]
    pub review: bool,
    /// When true, render the video via the Hyperframes service instead
    ...
    /// Optional per-composition step count override. `None` / `0` =
...
pub struct AudiobookSummary {
    ...
    pub is_short: bool,
    /// `true` when the book was generated by the songbook pipeline
    ...
    /// Number of song snippets requested at create time. Stays
...
pub struct UpdateAudiobookRequest {
    ...
    pub is_short: Option<bool>,
    /// Toggle songbook mode. Existing chapters are not regenerated —
    /// flip and regenerate the outline to take effect.
    pub is_songbook: Option<bool>,
    /// Update the snippet count. Existing snippets stay on disk;
...
impl DbAudiobook {
    fn to_summary(&self) -> Result<AudiobookSummary> {
        Ok(AudiobookSummary {
            ...
            is_songbook: self.is_songbook.unwrap_or(false),
            snippet_count: self
                .snippet_count
                .map(|n| n.clamp(0, 12) as u32)
                .unwrap_or(0),
...
pub async fn create(
    ...
    let is_songbook = body.is_songbook.unwrap_or(false);
    if is_short && is_songbook {
        return Err(Error::Validation(
            "is_short and is_songbook are mutually exclusive — songbooks need multiple chapters \
             to walk through verses, which a 90-second Short can't fit"
                .into(),
        )
        .into());
    }
    let snippet_count: i64 = body.snippet_count.map(|n| n.min(12) as i64).unwrap_or(0);
    if snippet_count > 0 && !is_songbook {
        return Err(Error::Validation(
            "snippet_count is only valid for songbooks (set is_songbook = true)".into(),
        )
        .into());
    }
    ...
    // Songbook snippets: independent of the auto-pipeline so users
    // get the audio clips even when they kick off the rest of the
    // flow manually. Two paths:
    //
    //   1. `preview_id` set — the user already vetted clips through
    //      the preview UI. Move those WAVs into the audiobook's
    //      snippet dir so the publish step splices the *exact*
    //      tracks the user listened to. No job, no second yt-dlp
    //      call, no risk of Tinyfish resolving to a different
    //      YouTube video the second time around.
    //   2. No preview — fall back to enqueuing the `SongSnippets`
    //      job, which runs the full pipeline asynchronously.
    //
    // Both branches are best-effort: a failed adopt-from-preview
    // logs a warn and queues the job as a fallback rather than
    // 500-ing the create.
    if is_songbook && snippet_count > 0 {
        let preview_id = body
            .preview_id
            .as_deref()
            .map(str::trim)
        ...
        if let Some(pid) = preview_id {
            match adopt_preview_snippets(&state, pid, &id, snippet_count as u32).await {
                Ok(n) if n > 0 => {
                    adopted = true;
                    tracing::info!(
                        audiobook_id = id,
                ...
                Ok(_) => {
                    warn!(
                        audiobook_id = id,
                        preview_id = pid,
                        "snippets: preview produced 0 clips, falling back to job"
                    );
                }
                Err(e) => {
                    warn!(
                        ...
                        audiobook_id = id,
                        preview_id = pid,
                        "snippets: adopt from preview failed, falling back to job"
                    );
                }
            }
        }
        if !adopted {
            ...
            if let Err(e) = state.jobs().enqueue(req).await {
                warn!(
                    error = %e,
                    audiobook_id = id,
                    "auto-pipeline: enqueue song_snippets failed"
                );
            }
        }
    }
...
pub async fn patch(
    ...
    if let Some(songbook) = body.is_songbook {
        state
            .db()
            .inner()
            .query(format!("UPDATE audiobook:`{id}` SET is_songbook = $s"))
            .bind(("s", songbook))
            .await
            .map_err(|e| Error::Database(format!("patch is_songbook: {e}")))?
            .check()
            .map_err(|e| Error::Database(format!("patch is_songbook: {e}")))?;
    }
    if let Some(n) = body.snippet_count {
        let clamped = n.min(12) as i64;
        state
            .db()
            .inner()
            .query(format!("UPDATE audiobook:`{id}` SET snippet_count = $n"))
            .bind(("n", clamped))
            .await
            .map_err(|e| Error::Database(format!("patch snippet_count: {e}")))?
            .check()
            .map_err(|e| Error::Database(format!("patch snippet_count: {e}")))?;
    }
...
/// Move snippet WAVs the user previewed (under `_preview/<preview_id>/`)
/// into the new audiobook's snippet dir. Returns how many were
/// adopted. Validates `preview_id` shape to avoid path traversal.
///
/// Best-effort: we move whichever indices `1..=count` are on disk. A
/// missing `snippet-3.wav` doesn't abort the others — the publisher
/// already tolerates gaps. Caller treats `Ok(0)` as "preview empty"
/// and falls back to enqueuing the regular snippet job.
async fn adopt_preview_snippets(
    state: &AppState,
    preview_id: &str,
    audiobook_id: &str,
    count: u32,
    ...
    if !preview_id.chars().all(|c| c.is_ascii_hexdigit()) || preview_id.len() != 32 {
        return Err(Error::Validation(format!(
            "invalid preview_id `{preview_id}`"
        )));
    }
    let storage = &state.config().storage_path;
    let preview_dir = storage.join("_preview").join(preview_id);
    let target_dir = storage.join(audiobook_id).join("snippets");
    tokio::fs::create_dir_all(&target_dir).await.map_err(|e| {
        Error::Other(anyhow::anyhow!(
            "create snippet target dir {}: {e}",
            target_dir.display()
        ))
    })?;

    let mut adopted = 0u32;
    for i in 1..=count.min(12) {
        let src = preview_dir.join(format!("snippet-{i}.wav"));
        if !src.exists() {
            continue;
        }
        let dst = target_dir.join(format!("snippet-{i}.wav"));
        // Try a rename first (cheap, atomic when src + dst share a
        // filesystem). Any failure (cross-device, permissions, …)
        // falls back to copy + remove so we still adopt the clip
        // even when storage layout splits `_preview/` from the
        // audiobook tree.
        ...
        match tokio::fs::copy(&src, &dst).await {
            ...
            Err(e) => {
                warn!(error = %e, src = ?src, dst = ?dst, "adopt: copy fallback failed");
            }
        }
    }
    // Best-effort cleanup; the dir may still hold extra clips beyond
    // `count`, in which case `remove_dir` errors and we leave them
