...
### Narrate tags
When creating a book, add tags to the book. These tags should be generated by the LLM model.
The X.ai LLM text to speech model will use these tags to generate the audiobook.
See: https://docs.x.ai/developers/model-capabilities/audio/text-to-speech#speech-tags

In the podcast list, show the duration of the podcasts and a filter on duration.

In the audiobook detail page, add a filter on the activity log to toggle the completed records, hide by default the completed ones

In the audiobook detail page, the category dropdown and podcast dropdown show the menu in white text, white background. It does not follow the theme.


Add an ideas page.
On this page you will search on X (twitter) for the latest trends and ideas for audiobooks.
You can also add your own ideas to the list.
Present this list in a table with the following columns:
- Idea
- Audiobook prompt
- Status (pending, in progress, completed)
- Date added
- Date completed
- Actions (delete, mark as completed)


Add to the ideas page:
- a search function for historical novels, fiction, non-fiction, etc. 
- link to open source websites like Project Gutenberg, Open Library, etc.

Instead of linking to the open source websites, we should have a button to import the books from these websites.


Add the option so that the audio (voices) can be generated by different voices depending on the contexts.
You can have narrator voices, female voices, male voices, etc.
This would be an optional feature that can be enabled in the audiobook settings.
...
Add the possibility to a a standard openai LLM (like self hosted ones on lmstudio).
In the add llm dialog box, add a tab for standard openai LLMs.
