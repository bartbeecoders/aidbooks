...
type WithArtStyle = {
   ...
   * the topic is treated as a song reference, lyrics + artist
   * info are fetched via Tinyfish, and the dedicated
   * `outline_songbook` prompt drives chapter planning. Mutually
   * exclusive with `is_short` server-side.
   */
  is_songbook?: boolean | null;
  /**
   * Number of song snippets (≤ 12) the songbook was created with.
   * The server resolves the song to a YouTube URL, downloads the
   * audio with yt-dlp, cuts N evenly-spaced ~12 s clips, and
   * intercalates them between chapters at YouTube publish time.
   * `0` (the only legal value when `is_songbook` is false)
   * disables the snippet job entirely.
   */
  snippet_count?: number | null;
  /**
   * Optional preview id from `POST /songbook/preview-snippets`. When
   * set, the create flow adopts those exact WAVs as the audiobook's
   * snippets and skips the background `SongSnippets` job — so the
   * published video uses the clips the user actually heard, not a
   * fresh search that might land on a different track.
   */
...
export interface AutoPublishPipeline {
   ...
   * content-aware scenes (title card, chapter intros, image + caption,
   * occasional pull-quote) with animated transitions instead of the
   * cover-loop slideshow.
   */
  hyperframes?: boolean | null;
  /**
   * Optional per-composition step count. `null` / `0` = auto-scale
   * by narration duration (~1 step / 15 s, capped at 120). Otherwise
   * clamped to `2..=120`. In playlist mode the value is per chapter.
   */
...
export interface PublishYoutubeRequest {
  ...
  /**
   * When true, render the visual track via the Hyperframes service:
   * title card + per-chapter intros + image-with-caption scenes +
   * pull-quote scenes for long-form, image-with-caption + opener for
   * Shorts. Mutually exclusive with `animate`.
   */
  hyperframes?: boolean | null;
  /**
   * Optional per-composition step count. `null` / `0` / `1` =
   * auto-scale (~1 step / 15 s, capped at 120). Otherwise clamped
   * to `2..=120`. In playlist mode the value is per chapter.
   */
  hyperframes_steps?: number | null;
  description?: string | null;
  /**
